...
## Streaming Frontend API

### Running the Web Interface

The module includes a FastAPI-based streaming chat frontend:
...
| `/chat/approve` | POST | Approve/deny sensitive actions |
| `/chat/history/{thread_id}` | GET | Get conversation history |
...
```json
{
    "message": "I need to book a flight to NYC",
    "passenger_id": "5102 899977",
    "thread_id": null  // Optional, auto-generated if null
}
```

**Response**: Server-Sent Events (SSE) stream with events:
- `message` - AI/assistant/human/tool message
- `interrupt` - Requires user approval for sensitive action
- `done` - Stream completed, returns `thread_id`
