...
def book_hotel(hotel_id: int, *, config: RunnableConfig) -> str:
    """Book a hotel by its ID for the current user."""
    configuration = config.get("configurable", {})
    passenger_id = configuration.get("passenger_id", None)
    if not passenger_id:
        raise ValueError("No passenger ID configured.")
    ...
    cursor.execute(
        "UPDATE hotels SET booked = 1, passenger_id = ? WHERE id = ?",
...
def update_hotel(
    ...
    *,
    ...
    # Check ownership
    cursor.execute("SELECT passenger_id FROM hotels WHERE id = ?", (hotel_id,))
    row = cursor.fetchone()
    if not row or row[0] != passenger_id:
        conn.close()
        return f"Hotel {hotel_id} not found or not booked by this user."
...
def cancel_hotel(hotel_id: int, *, config: RunnableConfig) -> str:
    ...
    cursor.execute("UPDATE hotels SET booked = 0, passenger_id = NULL WHERE id = ?", (hotel_id,))
    conn.commit()
    conn.close()
